...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.48\textwidth]{royg_stages.png}
\caption{Tomato ripening stages mapped to the Chromatic Index $X = G/(R+G)$. The continuous ROYG colour gradient corresponds to six USDA-equivalent stages, from Green ($X \approx 1.0$) to Red ($X \approx 0.0$). The RL policy targets $X \leq 0.15$ (Light Red–Red boundary) as the harvest threshold.}
...
where:
\begin{itemize}
    \item $k_1$: Cultivar-specific ripening rate constant, calibrated to $0.02$\,day$^{-1}\,^\circ$C$^{-1}$ based on empirical ripening times of 10--14 days at 20--25$^\circ$C (Section~\ref{subsec:ripening_kinetics}).
...
The ambient temperature mean of 27.0$^\circ$C reflects the annual mean for Iligan City, Philippines. A diurnal cycle with amplitude $\pm 4^\circ$C (peak at 14:00, trough at 02:00) explicitly models the indoor day/night swing (nighttime $\sim$23$^\circ$C, daytime $\sim$31$^\circ$C). The standard deviation of 2.0$^\circ$C captures the remaining stochastic variation (weather, cloud cover) above and below the diurnal envelope, without air conditioning. The ripening rate $k_1 = 0.02$\,day$^{-1}\,^\circ$C$^{-1}$ was calibrated from empirical data (Section~\ref{subsec:ripening_kinetics}), with domain randomization spanning $\pm 40\%$ to cover inter-cultivar variability between slow-ripening commercial hybrids (Diamante Max, $k_1 \approx 0.015$) and faster native varieties (Kamatis Tagalog, $k_1 \approx 0.028$).
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.48\textwidth]{reward_decomposition.png}
\caption{Reward function decomposition for a representative scenario ($X_0=0.9$, $k_1=0.02$, target=5\,d, $T=27^\circ$C). The quality component remains near zero while the fruit is unripe ($X > 0.15$), then rises sharply at the harvest threshold. The timing penalty forms a symmetric trough centred on the target day. The total reward (bold) is maximised by harvesting just before the target, balancing both components.}
