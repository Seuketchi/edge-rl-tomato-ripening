...
\midrule
Random & 0.930 & 1.01 & $-35.02 \pm 68.89$ \\
Fixed-Day & 0.927 & 0.95 & $-7.44 \pm 32.70$ \\
Fixed-Stage5 & 0.947 & 1.62 & $-2288.03 \pm 654.42$ \\
\textbf{Edge-RL (Ours)} & \textbf{0.905} & \textbf{0.55} & $\mathbf{-1.13 \pm 5.68}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{Edge-RL}: Achieves the lowest timing error (0.55 days) by dynamically adapting to the slower biological limits. Instead of aggressive heating, the agent learned to heavily bias cooling to prevent premature ripening, avoiding safety penalties entirely.
\end{itemize}
...
\begin{itemize}
    \item \textbf{HEAT (+1$^\circ$C)}: Elected in only 0.2\% of steps. The baseline ambient temperature ($27^\circ$C) is already sufficient to drive the slower biological kinetics.
    \item \textbf{MAINTAIN}: Chosen 6.5\% of the time, generally during the early phases of the episode.
    \item \textbf{COOL (-1$^\circ$C)}: Dominates the strategy (93.2\% frequency). The agent continuously cools the chamber below ambient to suppress the ripening rate, ensuring the chromatic index $X$ declines slowly enough to perfectly align with the target deadline.
\end{itemize}

This counter-intuitive behaviour underscores the utility of deep reinforcement learning in biological control applications: the agent discovered that in a tropical ambient environment ($27 \pm 3^\circ$C), the optimal path to extending shelf-life and hitting delayed harvest targets is continuous cooling, rather than the heuristic assumption of active heating.
