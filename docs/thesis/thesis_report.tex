...
% ==============================================================
\begin{abstract}
Post-harvest losses account for 20--40\% of tomato production in developing countries, driven by the absence of affordable, intelligent decision-support systems. Existing IoT solutions provide passive monitoring without autonomous action, while cloud-based reinforcement learning (RL) systems require expensive infrastructure and continuous connectivity. This thesis proposes \textit{Edge-RL}, a novel system that deploys a complete RL-based decision pipeline---from visual perception to autonomous ripening control---entirely on a \$33 ESP32-S3 microcontroller. The system extracts a Continuous Chromatic Index directly from raw RGB pixel statistics (mean, std, mode) and feeds a 16-dimensional state vector to a distilled Deep Q-Network policy. Direct pixel extraction eliminates covariate shift between simulator observations and real hardware. Hardware-level safety guardrails enforce biological temperature constraints ($12.5$--$35^{\circ}$C) independent of the learned policy. In digital twin simulations under calibrated ripening kinetics ($k_1=0.02$\,day$^{-1}\,^\circ$C$^{-1}$), the RL agent reduces harvest timing error to 0.79~days (vs. $\geq$1.21~days for heuristic baselines) with 100\% harvest rate. Policy distillation compresses the 270~KB teacher DQN into a 21.8~KB FP32 student MLP with 92.6\% action fidelity and $\sim$7\,ms ESP32-S3 inference latency. This work presents the first demonstration of a complete sim-to-edge RL pipeline for agricultural post-harvest optimization.
\end{abstract}
...
The teacher policy (DQN, 256$\times$256) is distilled into a student policy (MLP, 64$\times$64). As shown in Table~\ref{tab:distill_results}, the student achieves 92.6\% action fidelity while reducing model size by $\sim$92\%, making it deployable within the ESP32-S3's 512\,KB internal SRAM. Despite the fidelity gap, the student achieves a mean reward of $-5.44$ --- marginally better than the teacher's $-6.56$ --- with 100\% harvest rate.
...
Model Size & $\sim$270 KB (FP32) & $\sim$21.8 KB (FP32) & $-$92\% \\
...
The DQN policy (mean reward $-6.56 \pm 13.19$) was evaluated in the physics-based digital twin with domain randomization. The agent consistently modulates temperature---dominant cooling (86.6\%) with targeted heating (7.8\%) and maintain (5.5\%)---to drive $X$ toward the ripe threshold ($X \leq 0.15$) before the deadline, achieving 100\% harvest rate. Representative trajectories are shown in Fig.~\ref{fig:traj}.
...
Table~\ref{tab:baseline_comparison} summarizes performance metrics relative to baseline heuristics. The RL agent outperforms all baselines on timing error ($0.79$\,d vs. $\geq 1.21$\,d).
...
\textbf{Edge-RL} & \textbf{0.817} & \textbf{0.79} & $\mathbf{-6.56 \pm 13.19}$ \\
\bottomrule
\end{tabular}
...
The agent exhibits a cooling-dominant control strategy: \textbf{Cool} 86.6\%, \textbf{Heat} 7.8\%, \textbf{Maintain} 5.5\%. Under the calibrated ripening constant ($k_1=0.02$\,day$^{-1}\,^\circ$C$^{-1}$) and tropical ambient (27$^\circ$C), the agent discovered that continuous thermal suppression—not active heating—is the optimal path to hitting delayed harvest targets.
...
\midrule
Mean reward & $> -15$ & $-6.56$ & Eval \\
Timing error & $\pm$1 day & 0.79\,d & $|t_{\text{rem}}|$ \\
Quality & $>$ 0.80 & 0.817 & $1{-}|X{-}X_t|$ \\
