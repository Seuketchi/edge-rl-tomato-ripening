...
\section{Policy Distillation Results}
    ...
    \begin{figure}[ht]
        \centering
        \includegraphics[width=0.85\textwidth]{reward_histogram.png}
        \caption{Episode reward distribution of the DQN teacher policy across 100 evaluation episodes (left). The distribution is right-skewed, with the majority of episodes achieving rewards between $+2$ and $+6$. The timing error distribution (right) peaks near 0.5--1.0 days, confirming that the policy consistently harvests within practical scheduling tolerance.}
...
\section{Emergent Behaviour Analysis}
    ...
    \begin{figure}[ht]
        \centering
        \includegraphics[width=0.85\textwidth]{policy_heatmap.png}
        \caption{Policy decision heatmap visualising the DQN agent's action selection across the 2D state space of Chromatic Index $X$ (y-axis) and remaining days until target harvest (x-axis). HEAT (red) dominates when $X$ is high (green, unripe) and time is short; COOL (blue) is preferred when $X$ is close to threshold and time budget remains; MAINTAIN (grey) bridges transitions. The emergent diurnal boundary reflects the agent's learned trade-off between ripening acceleration and deadline precision.}
