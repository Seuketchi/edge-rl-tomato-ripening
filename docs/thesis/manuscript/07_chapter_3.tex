...
\section{Phase 2: Digital Twin Construction}
    ...
    \begin{figure}[ht]
        \centering
        \includegraphics[width=0.85\textwidth]{royg_stages.png}
        \caption{Continuous Chromatic Index scale showing the ROYG progression from fully green ($X = 1.0$, mature green) to fully red ($X = 0.0$, ripe). The harvest threshold at $X = 0.15$ corresponds to the red-ripe stage; the biological base temperature $T_{\text{base}} = 12.5^{\circ}$C marks the lower bound below which ripening effectively ceases.}
        \label{fig:royg_stages}
    \end{figure}
...
\section{Phase 3: Reinforcement Learning Formulation and Teacher Training}
    ...
    \begin{figure}[ht]
        \centering
        \includegraphics[width=0.85\textwidth]{reward_decomposition.png}
        \caption{Decomposition of the composite reward signal as a function of harvest day relative to the 5-day target. The quality component $r_{\text{progress}}$ increases monotonically with ripening progress. The timing penalty scales with deviation from the target, dominating as the deadline passes. The total reward peaks at exactly the target day, incentivising precise harvest scheduling.}
...
\section{Phase 4: Policy Distillation and Model Compression}
    ...
    \begin{enumerate}
        ...
        \item \textbf{Data Generation:} The trained teacher generates $N = 100{,}000$ state-action pairs $(s_i, a_i^{*})$ by rolling out the deterministic greedy policy in the simulator. For each state $s_i$, the teacher's greedy action $a_i^{*} = \arg\max_a Q_{\theta}(s_i, a)$ is recorded as a hard label. It is important to note that these 100,000 rollout samples naturally reflect the teacher's action distribution; if one action (e.g., COOL) dominates the optimal policy, its corresponding states will be overrepresented in the distillation dataset, which can bias the student's per-class action fidelity.

        \item \textbf{Student Training:} A compact MLP with $[64, 64]$ hidden layers (5,443 parameters) is trained to minimize the cross-entropy loss between the student's predicted action distribution and the teacher's hard action labels:

        \begin{equation}
            \mathcal{L} = -\frac{1}{N}\sum_{i=1}^{N} \log \hat{p}(a_i^{*} \mid s_i)
            \label{eq:distill_loss}
        \end{equation}

        \noindent where $a_i^{*} = \arg\max_a Q_{\theta}(s_i, a)$ is the teacher's greedy action for state $s_i$, and $\hat{p}(a \mid s)$ is the student's softmax output. This behavioural cloning formulation is sufficient because the teacher's policy is deterministic (greedy Q-value selection), making soft-target distillation unnecessary.

        \item \textbf{Export:} The student weights are exported as FP32 C constant arrays (\texttt{policy\_weights.h}) for direct compilation into the ESP32-S3 firmware, alongside 20 golden verification vectors (\texttt{golden\_vectors.h}) for on-device correctness checking at boot. The total weight file size is approximately 22~KB, well within the 16~MB flash budget. INT8 quantization was evaluated but not adopted in the final deployment due to unacceptable fidelity degradation ($\sim$74\% action agreement versus 97.8\% at FP32), and because the ESP32-S3 lacks a hardware integer MAC accelerator that would offset the precision loss with a speed advantage.
    \end{enumerate}
