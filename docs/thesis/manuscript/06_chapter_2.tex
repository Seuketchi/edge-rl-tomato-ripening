...
\section{Edge AI and Model Compression}
    ...
    Symmetric per-channel quantization---where independent scaling factors are computed for each output neuron---provides a favourable balance between compression ratio and accuracy preservation for small MLP architectures. Libraries such as ESP-DL \citep{espdl2024} and TensorFlow Lite for Microcontrollers (TFLite Micro) provide hardware-optimized INT8 inference kernels for Espressif microcontrollers.

    However, for sufficiently compact models (fewer than 10,000 parameters), an alternative approach bypasses ML runtimes entirely: the distilled student weights are exported as FP32 C constant arrays and compiled directly into the firmware. This eliminates runtime overhead and library dependencies at the cost of a 4$\times$ larger weight footprint compared to INT8---a trade-off that is acceptable when the model fits comfortably within available flash memory. The present study adopts this pure-C FP32 approach, achieving sub-10~ms inference latency on the ESP32-S3 without requiring any external ML library.
